...
function renderDone(app){
  ...
  app.innerHTML=`<section class="card"><h1>結果: ${ok} / ${deck.length} 正解</h1><p>${wrongIds.length ? '不正解だけをまとめた復習リンクを作りました。共有・保存して後で解き直せます。' : '全問正解です。別の10問にも挑戦できます。'}</p><div class="review-box ${wrongIds.length?'show':''}"><label for="shareUrl"><b>復習シェアリンク</b></label><input id="shareUrl" class="share-url" readonly value="${esc(review)}"><div class="flip-controls"><button class="btn" id="copyBtn">リンクをコピー</button><a class="ghost-btn" href="${esc(review)}">この不正解だけ復習</a></div></div><div class="flip-controls"><button class="btn" id="againBtn">新しい10問</button><a class="ghost-btn" href="/today/">今日の問題</a><a class="ghost-btn" href="/topics/hikkake/">ひっかけ問題</a></div></section>`;
